...
Regardless of whether a standard soldering iron or a dedicated press is used, a soldering tip sized specifically for the insert's diameter is needed for a clean, controlled installation — a standard conical or chisel tip will not seat the insert evenly. See Commercial Sources below for a sourced set of insert-sized tips; note that the heat-set insert press linked there already includes a set of these tips.
