...
`AMD_Xilinx_ZCU216_Board.SLDPRT` — manufacturer-supplied CAD model of the AMD Xilinx ZCU216 RFSoC development platform.

**MIT Haystack RF Interface Board**
`MIT_Haystack_RF_Interface_Board.SLDPRT` — high-fidelity CAD model of the RF Interface Board developed by Frank Lind at MIT Haystack Observatory.

**Simplified RF Interface Board**
`MIT_Haystack_RF_Interface_Board_Simplified.SLDPRT` — simplified reference model developed specifically for enclosure design. Only mechanically significant geometry was retained in order to improve SolidWorks performance while preserving:
...
The simplified models remove unnecessary cosmetic and internal features while preserving all mechanically significant geometry. Using simplified reference models substantially improves CAD performance, rebuild times, and assembly responsiveness without affecting enclosure design accuracy.
...
Both cooling fans are mounted directly to the enclosure side panels. This arrangement simplifies assembly while providing effective airflow through the enclosure.
...
Frank Lind, MIT Haystack Observatory — RF Interface Board

**Manufacturing**
Protocase — custom precision sheet metal fabrication.